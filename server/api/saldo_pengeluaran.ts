...
export default defineEventHandler(async (event) => {
  ...
  if (event.method === 'GET' && query.type === 'saldo') {
    const saldoList = await db.execute(sql`
      SELECT
        d.id AS rkakl_detail_id,
        d.akun_id,
        d.jumlah AS total_anggaran,
  ...
  // Filter khusus status_pengajuan_id = 2 (cair) saja
  where.push(eq(pengajuan.status_pengajuan_id, 2));
  ...
  const data = result.map((row) => {
    const pengeluaran = pengeluaranMap.get(row.id) || null;
    // saldo = rkakl_jumlah - jumlah_pengeluaran
    const saldo = Number(row.rkakl_jumlah || 0) - Number(pengeluaran?.jumlah_pengeluaran || 0);
    return {
      ...row,
      pengeluaran,
      saldo,
  ...
  const totalResult = await db.select({ count: sql<number>`count(*)` }).from(pengajuan).where(where.length ? and(...where) : undefined);
  const total = totalResult && totalResult[0] ? totalResult[0].count : 0;
