...
export default defineEventHandler(async (event) => {
	// Ambil ID satker dari path (parameter URL) e.g., /api/dashboard/unit_kerja/satker/1
	...
	// Ambil data unit (wajib difilter berdasarkan satkerId)
	const unitRows = await db.select().from(unit).where(eq(unit.satker_id, satkerId));

	// Ambil pagu dari anggaran_suboutput (wajib difilter berdasarkan satkerId)
	const anggaranRows = await db.select().from(anggaranSuboutput).where(eq(anggaranSuboutput.satker_id, satkerId));

	// Ambil realisasi dari pengajuan (wajib difilter berdasarkan satkerId)
