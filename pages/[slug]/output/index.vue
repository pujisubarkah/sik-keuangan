              ...
              <tr v-for="(item, index) in paginatedData" :key="`${item.output_id}-${item.satker_id}-${item.tahun_anggaran_id}`" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    ...
                    <NuxtLink :to="`/output/update?id=${item.output_id}`" class="text-yellow-600 hover:text-yellow-900 bg-yellow-50 hover:bg-yellow-100 p-2 rounded-md transition-colors" title="Edit">
...
const fetchOutput = async () => {
  ...
  try {
    const response = await fetch('/api/anggaran_suboutput/by-output', { headers })
