              ...
              <tr v-for="(item, index) in filteredData" :key="item.id" class="hover:bg-blue-50 transition-all duration-150">
                ...
                <td class="text-center">
                  <div class="flex gap-1 justify-center">
                    <!-- Icon Mata (Lihat) -->
                    <NuxtLink :to="`/${$route.params.slug}/kegiatan/view/${item.id}`" class="btn btn-ghost">
                      <Icon icon="tabler:eye" class="w-6 h-6" />
                    </NuxtLink>
                    <!-- Icon Pensil (Update) -->
                    <NuxtLink :to="`/${$route.params.slug}/kegiatan/update/${item.id}`" class="btn btn-xs btn-circle btn-warning tooltip" data-tip="Update">
                      <Icon icon="tabler:pencil" class="w-5 h-5" />
                    </NuxtLink>
                    <!-- Icon Tempat Sampah (Delete) -->
