...
const judul = ref('tes')
const berkas = ref('Penawaran Pembuatan Aplikasi Sistem Informasi Akademik.pdf')
const user_pembuat = ref('Admin')
...
async function submitForm() {
  ...
  setTimeout(() => {
    ...
    toastMessage.value = 'Berkas laporan berhasil diperbarui'
  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50 pt-16 pb-24">
    ...
    <div class="max-w-5xl mx-auto px-6 mb-8">
      <nav class="flex items-center gap-2 text-sm">
        ...
        <NuxtLink :to="`/${route.params.slug}/berkas-laporan`" class="text-slate-500 hover:text-blue-600 transition-colors font-medium">Berkas Laporan</NuxtLink>
        <Icon icon="mdi:chevron-right" class="w-4 h-4 text-slate-300" />
    ...
    <div class="max-w-5xl mx-auto px-6 mb-8">
      <div class="flex items-start justify-between">
        <div>
          <h1 class="text-3xl font-bold bg-gradient-to-r from-blue-700 to-indigo-600 bg-clip-text text-transparent">Update Berkas Laporan</h1>
          <p class="text-slate-500 mt-2 text-base">Ubah data berkas laporan sesuai kebutuhan.</p>
        </div>
      </div>
    </div>
        ...
        <form class="px-8 py-8" @submit.prevent="submitForm">
          <div class="space-y-7">
            <div class="flex items-center gap-4">
              <label class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">Judul <span class="text-rose-500">*</span></label>
              <TextField v-model="judul" type="text" required class="flex-1 [&>input]:rounded-xl [&>input]:border-slate-200 [&>input]:bg-slate-50/50 [&>input]:px-4 [&>input]:py-3 [&>input:focus]:border-blue-400 [&>input:focus]:ring-4 [&>input:focus]:ring-blue-100/60" />
            </div>
            <div class="flex items-center gap-4">
              <label class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">File Berkas <span class="text-rose-500">*</span></label>
              <TextField v-model="berkas" type="text" required class="flex-1 [&>input]:rounded-xl [&>input]:border-slate-200 [&>input]:bg-slate-50/50 [&>input]:px-4 [&>input]:py-3 [&>input:focus]:border-blue-400 [&>input:focus]:ring-4 [&>input:focus]:ring-blue-100/60" />
            </div>
            <div class="flex items-center gap-4">
              <label class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">User Pembuat</label>
              <TextField v-model="user_pembuat" type="text" readonly class="flex-1 [&>input]:rounded-xl [&>input]:border-slate-200 [&>input]:bg-gray-100 [&>input]:px-4 [&>input]:py-3" />
            </div>
          </div>
