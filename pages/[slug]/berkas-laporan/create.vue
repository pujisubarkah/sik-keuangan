...
async function submitForm() {
  ...
  setTimeout(() => {
    ...
    toastMessage.value = 'Berkas laporan berhasil ditambahkan'
  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50 pt-16 pb-24">
    ...
    <div class="max-w-5xl mx-auto px-6 mb-8">
      <nav class="flex items-center gap-2 text-sm">
        ...
        <NuxtLink :to="`/${route.params.slug}/berkas-laporan`" 
      ...
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="text-slate-500 mt-2 text-base">
            Lengkapi formulir di bawah untuk menambahkan data berkas laporan baru ke dalam sistem.
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-200/80 overflow-hidden">
        ...
        <div class="px-8 py-6 bg-gradient-to-r from-blue-600/5 to-indigo-600/5 border-b border-slate-100">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold text-slate-800">Detail Berkas Laporan</h2>
        ...
        <form class="px-8 py-8" @submit.prevent="submitForm">
          <div class="space-y-7">
            ...
            <div class="flex items-center gap-4">
              <label for="judul" class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">
                <Icon icon="mdi:format-title" class="w-4 h-4 text-slate-400" />
                Judul <span class="text-rose-500">*</span>
              </label>
              <TextField v-model="judul" id="judul" type="text" required maxlength="255"
            ...
            <!-- Berkas -->
            <div class="flex items-center gap-4">
              <label for="berkas" class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">
                <Icon icon="mdi:file-upload-outline" class="w-4 h-4 text-slate-400" />
                File Berkas <span class="text-rose-500">*</span>
              </label>
              <TextField v-model="berkas" id="berkas" type="text" required maxlength="255"
                         class="flex-1 [&>input]:rounded-xl [&>input]:border-slate-200 [&>input]:bg-slate-50/50 [&>input]:px-4 [&>input]:py-3 
                                [&>input]:text-slate-700 [&>input]:placeholder-slate-400 
                                [&>input:focus]:border-blue-400 [&>input:focus]:ring-4 [&>input:focus]:ring-blue-100/60 [&>input:focus]:bg-white 
                                [&>input]:transition-all [&>input]:duration-200"
                         placeholder="Nama/URL berkas atau klik untuk upload..." />
            </div>

            <!-- User Pembuat -->
            <div class="flex items-center gap-4">
              <label for="user_pembuat" class="w-48 flex items-center gap-2 text-sm font-semibold text-slate-700 flex-shrink-0">
                <Icon icon="mdi:account" class="w-4 h-4 text-slate-400" />
                User Pembuat <span class="text-rose-500">*</span>
              </label>
              <TextField v-model="user_pembuat" id="user_pembuat" type="text" required maxlength="255"
                         class="flex-1 [&>input]:rounded-xl [&>input]:border-slate-200 [&>input]:bg-slate-50/50 [&>input]:px-4 [&>input]:py-3 
                                [&>input]:text-slate-700 [&>input]:placeholder-slate-400 
                                [&>input:focus]:border-blue-400 [&>input:focus]:ring-4 [&>input:focus]:ring-blue-100/60 [&>input:focus]:bg-white 
                                [&>input]:transition-all [&>input]:duration-200"
