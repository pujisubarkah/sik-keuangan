<!-- filepath: c:\Users\user\Documents\sik-keuangan2\pages\[slug]\informasi\[id]\edit.vue -->
<template>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">
          Perbarui judul dan deskripsi informasi yang sudah ada.
        </p>
      </div>
      ...
      <div class="bg-white shadow overflow-hidden sm:rounded-lg border border-gray-200">
        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <h2 class="text-lg font-semibold text-gray-900">Formulir Edit</h2>
        </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="flex justify-end pt-4 border-t border-gray-200">
              <button 
                ...
                class="inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-semibold rounded-md shadow text-white"
                style="background-color:#2663A3;"
                @mouseover="(e) => e.currentTarget.style.backgroundColor = '#1F4F85'"
                @mouseleave="(e) => e.currentTarget.style.backgroundColor = '#2663A3'"
                :style="submitting || !form.judul.trim() || !form.deskripsi.trim() ? 'background-color:#d1d5db;color:#6b7280;cursor:not-allowed;' : 'background-color:#2663A3;color:#fff;'"
              >
...
const fetchInformasi = async () => {
  try {
    ...
    if (!response.ok) throw new Error('Gagal mengambil data informasi')
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/informasi/${route.params.id}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || 'Gagal mengupdate informasi')
    }
    ...
    alert('Gagal mengupdate informasi: ' + error.message)
  } finally {
